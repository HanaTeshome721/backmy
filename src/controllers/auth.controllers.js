...
const generateAccessAndRefreshTokens=async(userId)=>{
    try{ 
        ...
        return {accessToken,refreshToken}
            ...
            throw new ApiError( 500,  error.message || "something is wrong when generating token")
    }
}
    ...
    if(!createdUser){
        ...
        .json(
            new ApiResponse(
                200,
                {user:createdUser},
                "User registered successfully and verification email is sent to ur emial "
            )
        )
})
...
const verifyEmail =asyncHandler(async(req,res)=>{
    ...
    let hashedToken=crypto
     ...
     const user=  await User.findOne({
            emailVerificationExpiry:hashedToken,
...
const  refreshAcessToken =asyncHandler(async(req,res)=>{
    ...
    if(!incomingRefreshToken){
        throw new ApiError(401,"unauthirizes access")
    }
    ...
    try{
      ...
      if(incomingRefreshToken !==user?.refreshToken){
        throw new ApiError(401,"refresh token in expires")
      }
